SECTION "Tilesets 1", ROMX

Overworld_GFX::     INCBIN "gfx/tilesets/overworld.2bpp"
;Overworld_GFX::
;	INCBIN "gfx/tilesets/overworld.2bpp", $0, tile $80
;	INCBIN "gfx/tilesets/overworld.2bpp", tile $100
Overworld_Block::   INCBIN "data/tilesets/overworld_metatiles.bst"
Overworld_Attr::    INCBIN "data/tilesets/overworld_attributes.atr"

RedsHouse1_GFX::
RedsHouse2_GFX::    INCBIN "gfx/tilesets/reds_house.2bpp"
RedsHouse1_Block::
RedsHouse2_Block::  INCBIN "data/tilesets/reds_house_metatiles.bst"
RedsHouse1_Attr::
RedsHouse2_Attr::   INCBIN "data/tilesets/reds_house_attributes.atr"

House_GFX::         INCBIN "gfx/tilesets/house.2bpp"
House_Block::       INCBIN "data/tilesets/house_metatiles.bst"
House_Attr::        INCBIN "data/tilesets/house_attributes.atr"

Mansion_GFX::       INCBIN "gfx/tilesets/mansion.2bpp"
Mansion_Block::     INCBIN "data/tilesets/mansion_metatiles.bst"
Mansion_Attr::      INCBIN "data/tilesets/mansion_attributes.atr"

SECTION "Tilesets 1bis", ROMX

ShipPort_GFX::      INCBIN "gfx/tilesets/ship_port.2bpp"
ShipPort_Block::    INCBIN "data/tilesets/ship_port_metatiles.bst"
ShipPort_Attr::     INCBIN "data/tilesets/ship_port_attributes.atr"

Interior_GFX::      INCBIN "gfx/tilesets/interior.2bpp"
Interior_Block::    INCBIN "data/tilesets/interior_metatiles.bst"
Interior_Attr::     INCBIN "data/tilesets/interior_attributes.atr"

Plateau_GFX::       INCBIN "gfx/tilesets/plateau.2bpp"
Plateau_Block::     INCBIN "data/tilesets/plateau_metatiles.bst"
Plateau_Attr::      INCBIN "data/tilesets/plateau_attributes.atr"


SECTION "Tilesets 2", ROMX

Dojo_GFX::
Gym_GFX::           INCBIN "gfx/tilesets/gym.2bpp"
Dojo_Block::
Gym_Block::         INCBIN "data/tilesets/gym_metatiles.bst"
Dojo_Attr::
Gym_Attr::          INCBIN "data/tilesets/gym_attributes.atr"

Mart_GFX::
Pokecenter_GFX::    INCBIN "gfx/tilesets/pokecenter.2bpp"
Mart_Block::
Pokecenter_Block::  INCBIN "data/tilesets/pokecenter_metatiles.bst"
Mart_Attr::
Pokecenter_Attr::   INCBIN "data/tilesets/pokecenter_attributes.atr"

ForestGate_GFX::
;Museum_GFX::
Gate_GFX::          INCBIN "gfx/tilesets/gate.2bpp"
ForestGate_Block::
;Museum_Block::
Gate_Block::        INCBIN "data/tilesets/gate_metatiles.bst"
ForestGate_Attr::
;Museum_Attr::
Gate_Attr::         INCBIN "data/tilesets/gate_attributes.atr"


SECTION "Tilesets 2bis", ROMX

Forest_GFX::        INCBIN "gfx/tilesets/forest.2bpp"
Forest_Block::      INCBIN "data/tilesets/forest_metatiles.bst"
Forest_Attr::       INCBIN "data/tilesets/forest_attributes.atr"

Facility_GFX::      INCBIN "gfx/tilesets/facility.2bpp"
Facility_Block::    INCBIN "data/tilesets/facility_metatiles.bst"
Facility_Attr::     INCBIN "data/tilesets/facility_attributes.atr"


SECTION "Tilesets 3", ROMX

Cemetery_GFX::      INCBIN "gfx/tilesets/cemetery.2bpp"
Cemetery_Block::    INCBIN "data/tilesets/cemetery_metatiles.bst"
Cemetery_Attr::     INCBIN "data/tilesets/cemetery_attributes.atr"

Cavern_GFX::        INCBIN "gfx/tilesets/cavern.2bpp"
Cavern_Block::      INCBIN "data/tilesets/cavern_metatiles.bst"
Cavern_Attr::       INCBIN "data/tilesets/cavern_attributes.atr"

Lobby_GFX::         INCBIN "gfx/tilesets/lobby.2bpp"
Lobby_Block::       INCBIN "data/tilesets/lobby_metatiles.bst"
Lobby_Attr::        INCBIN "data/tilesets/lobby_attributes.atr"

SECTION "Tilesets 3bis", ROMX

Ship_GFX::          INCBIN "gfx/tilesets/ship.2bpp"
Ship_Block::        INCBIN "data/tilesets/ship_metatiles.bst"
Ship_Attr::         INCBIN "data/tilesets/ship_attributes.atr"

Lab_GFX::           INCBIN "gfx/tilesets/lab.2bpp"
Lab_Block::         INCBIN "data/tilesets/lab_metatiles.bst"
Lab_Attr::          INCBIN "data/tilesets/lab_attributes.atr"

Club_GFX::          INCBIN "gfx/tilesets/club.2bpp"
Club_Block::        INCBIN "data/tilesets/club_metatiles.bst"
Club_Attr::         INCBIN "data/tilesets/club_attributes.atr"

Underground_GFX::   INCBIN "gfx/tilesets/underground.2bpp"
Underground_Block:: INCBIN "data/tilesets/underground_metatiles.bst"
Underground_Attr::  INCBIN "data/tilesets/underground_attributes.atr"


SECTION "Tilesets 4", ROMX
Museum_GFX::        INCBIN "gfx/tilesets/museum.2bpp"
Museum_Block::      INCBIN "data/tilesets/museum_metatiles.bst"
Museum_Attr::       INCBIN "data/tilesets/museum_attributes.atr"
