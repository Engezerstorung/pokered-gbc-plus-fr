...
FuchsiaCityGraphicSwapScript::
	; to check the Font isnt loaded
	ld hl, wFontLoaded
	bit 0, [hl]
	; save the Font loading state flag
	push af
	; Replace Swimmer with Lapras
	ld de, LaprasSprite 
	ld hl, vSprites tile $60
	lb bc, BANK(LaprasSprite), 12
	call CopyVideoData
	; check if the Font isnt loaded
	jr nz, .checkfossils
	ld de, LaprasSprite tile 12
...
.checkfossils
	...
	jr z, .nextfossil
	; Replace CoolTrainerM with Omanyte
	ld de, OmanyteSprite 
	ld hl, vSprites tile $18
	lb bc, BANK(OmanyteSprite), 12
	call CopyVideoData
	; check if the Font isnt loaded
	pop af
	ret nz
	ld de, OmanyteSprite tile 12
...
.nextfossil
	...
	jr z, .nofossil
	; Replace CoolTrainerM with Kabuto
	ld de, KabutoSprite 
	ld hl, vSprites tile $18
	lb bc, BANK(KabutoSprite), 12
	call CopyVideoData
	; check if the Font isnt loaded
	pop af
	ret nz
	ld de, KabutoSprite tile 12
	ld hl, vSprites tile $98
	lb bc, BANK(KabutoSprite), 12
	call CopyVideoData
	ret
.nofossil
	; Replace CoolTrainerM with Blank Sprite
	ld de, BlankSprite 
	ld hl, vSprites tile $18
	lb bc, BANK(BlankSprite), 12
	call CopyVideoData
	; check if the Font isnt loaded
	pop af
	ret nz
	ld de, BlankSprite tile 12
	ld hl, vSprites tile $98
	lb bc, BANK(BlankSprite), 12
	call CopyVideoData
	ret
