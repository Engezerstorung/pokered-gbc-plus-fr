...
; prepare the value to go to the second row starting position
	ld a, l
	add BG_MAP_WIDTH
	ld c, a
; prepare the value needed to wrap around the right edge to the left  if necessary
	ld a, l
	and %11100000
	ld b, a
...
REPT SCREEN_WIDTH / 2 - 1
	...
	ld [hli], a
	ld [hl], d
; the following 5 lines wrap us from the right edge to the left edge if necessary
	ld a, l
	inc a
	and %00011111
	or b
...
ENDR
	...
	ld l, c ; go to the second row starting position
...
; prepare the values needed to wrap us from bottom to top if necessary
	lb bc, $3, $98
